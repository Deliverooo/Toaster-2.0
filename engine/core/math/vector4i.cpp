...
namespace tst
{
	...
	constexpr Vector4I Vector4I::operator*(int32 val) const
	{
		return {x * val, y * val, z * val, w * val};
	}

	constexpr void Vector4I::operator*=(int32 val)
	{
		x *= val;
		y *= val;
		z *= val;
	...
	float32 Vector4I::length() const
	{
		return math::sqrt(static_cast<float>(x * x + y * y + z * z + w * w));
	}

	float32 Vector4I::dot(const Vector4I &v) const
	{
		return x * v.x + y * v.y + z * v.z + w * w;
	}

	float32 Vector4I::angle(const Vector4I &v) const
	{
