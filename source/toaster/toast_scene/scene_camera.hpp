...
namespace toaster
{
	...
	public:
		...
		void                  setPerspectiveFov(const float32 p_fov) { m_perspectiveFov = p_fov; }
		[[nodiscard]] float32 getPerspectiveFov() const { return m_perspectiveFov; }
		void                  setPerspectiveNearClip(const float32 p_z_near) { m_perspectiveNear = p_z_near; }
		[[nodiscard]] float32 getPerspectiveNearClip() const { return m_perspectiveNear; }
		void                  setPerspectiveFarClip(const float32 p_z_far) { m_perspectiveFar = p_z_far; }
		[[nodiscard]] float32 getPerspectiveFarClip() const { return m_perspectiveFar; }

		void                  setOrthographicSize(const float32 p_size) { m_orthoSize = p_size; }
		[[nodiscard]] float32 getOrthographicSize() const { return m_orthoSize; }
		void                  setOrthographicNearClip(const float32 p_z_near) { m_orthoNear = p_z_near; }
		[[nodiscard]] float32 getOrthographicNearClip() const { return m_orthoNear; }
		void                  setOrthographicFarClip(const float32 p_z_far) { m_orthoFar = p_z_far; }
		[[nodiscard]] float32 getOrthographicFarClip() const { return m_orthoFar; }

		void                          SetProjectionType(EProjectionType p_type) { m_projectionType = p_type; }
