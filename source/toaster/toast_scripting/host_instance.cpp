...
namespace toaster
{
	HostInstance::HostInstance(const HostInstanceSpecInfo &p_spec_info) : m_specInfo(p_spec_info)
	{
		...
		auto init_fptr{toaster::os::getProcAddress<hostfxr_initialize_for_runtime_config_fn>(m_hostfxrLibraryHandle, "hostfxr_initialize_for_runtime_config")};
		...
		auto close_fptr{toaster::os::getProcAddress<hostfxr_close_fn>(m_hostfxrLibraryHandle, "hostfxr_close")};

		void *load_assembly_and_get_function_pointer = nullptr;

		init_fptr(m_specInfo.configPath.wstring().c_str(), nullptr, &m_hostfxrContext);
