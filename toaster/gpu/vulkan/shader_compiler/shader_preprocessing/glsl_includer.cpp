...
namespace tst
{
	...
	shaderc_include_result *GlslIncluder::GetInclude(const char *requestedPath, const shaderc_include_type type, const char *requestingPath, const size_t includeDepth)
	{
		...
		if (source.empty())
		{
			...
			if (source.empty())
				TST_ERROR("Failed to load included file: {} in {}.", requestedFullPath.string(), requestingPath);
			sourceHash = hash::fnvHash(source.c_str());

			// Can clear "source" in case it has already been included in this stage and is guarded.
			stages = ShaderPreprocessor::preprocessHeader<EShaderLanguage::eGLSL>(source, isGuarded, m_ParsedSpecialMacros, m_includeData, requestedFullPath);
		}
		else if (isGuarded)
		{
			source.clear();
