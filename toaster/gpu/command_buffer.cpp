...
namespace tst
{
	...
	CommandBuffer::CommandBuffer(uint32 count, String debug_name, bool owned_by_swapchain) : m_debugName(std::move(debug_name))
	{
		if (count == 0)
		{
	...
	{
		TST_INFO_TAG("Renderer", "Submitting Render Command Buffer {}", m_debugName);
	...
	void CommandBuffer::_wait(vk::Semaphore waitSemaphore)
	{
		auto device = static_cast<nvrhi::vulkan::IDevice *>(Application::getGraphicsDevice().Get());
		lockGraphicsQueue();
	...
	void CommandBuffer::_commitGraphicsState()
	{
		m_activeCommandBuffer->setGraphicsState(m_graphicsState);
	}

	void CommandBuffer::_commitComputeState()
	{
