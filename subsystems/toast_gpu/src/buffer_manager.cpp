...
{
	BufferManager::BufferManager(Device &p_gpu_ctx) : m_gpuCtx(&p_gpu_ctx)
	{
		...
		m_pool.setDestroyCallback(+[](void *p_user_data, BufferHandle p_handle) -> void
		{
			...
			ts->m_gpuCtx->submitDeletion([allocator = ts->m_gpuCtx->getAllocator().getAllocator(), buffer_data]() mutable noexcept -> void
			{
	...
	BufferManager::~BufferManager()
	{
		...
		for (uint32 i{0u}; i < m_pool.getSize(); ++i)
		{
			if (m_pool._alive[i])
			{
				BufferData buffer_data{m_pool._data[i]}; // Copy
				TST_PERMA_ASSERT(buffer_data.buffer && buffer_data.allocation);
				m_gpuCtx->submitDeletion([allocator = m_gpuCtx->getAllocator().getAllocator(), buffer_data
										 ]() mutable noexcept -> void // The allocator must be destroyed after the deletion queue is executed
										 {
	...
	auto BufferManager::createBuffer(const BufferDesc &p_desc) -> BufferHandle
	{
		...
		if (buffer_data.memoryProperties == EMemoryProperties::eDeviceLocal)
			m_gpuCtx->getAllocator().createBuffer(buffer_data.size, buffer_data.usageFlags, 0u, buffer_data.buffer, buffer_data.allocation);
		else if (buffer_data.memoryProperties == EMemoryProperties::eHostVisibleCoherent)
		{
			m_gpuCtx->getAllocator().createBuffer(buffer_data.size, buffer_data.usageFlags,
		...
		if (p_desc.usageFlags & vk::BufferUsageFlagBits::eShaderDeviceAddress)
			buffer_data.address = m_gpuCtx->getBufferAddress(buffer_data.buffer);
