...
namespace toaster::gpu
{
	...
	public:
		...
		MeshManager(Device &p_gpu_ctx, BufferManager &p_buffer_manager, MaterialManager &p_material_manager, uint64 p_static_mesh_vertex_buffer_size_bytes,
		...
		[[nodiscard]] auto createStaticMesh(vk::CommandBuffer               p_cmd, const std::vector<StaticMeshVertex> &p_vertices, const std::vector<uint32> &p_indices,
		...
		auto getStaticMeshGlobalVertexBuffer() const -> vk::Buffer { return m_bufferManager->getBufferBuffer(m_staticMeshVertexBuffer); }
		auto getStaticMeshGlobalIndexBuffer() const -> vk::Buffer { return m_bufferManager->getBufferBuffer(m_staticMeshIndexBuffer); }
