...
namespace toaster::render
{
	...
	DynamicMaterialList::DynamicMaterialList(RenderContext &p_render_ctx) : m_renderCtx(&p_render_ctx)
	{
	}

	auto DynamicMaterialList::addMaterial(uint32 p_index, const String &p_name) -> DynamicMeshMaterialData &
	{
		auto &mat{m_materialDatas[p_index]};
		mat.name = p_name;
		return mat;
	...
	auto DynamicMaterialList::hasMaterial(uint32 p_index) const -> bool
	{
		return m_materialDatas.contains(p_index);
	}

	auto DynamicMaterialList::getMaterial(uint32 p_index) const -> const DynamicMeshMaterialData &
	{
		return m_materialDatas.at(p_index);
	}

	auto DynamicMaterialList::getMaterial(uint32 p_index) -> DynamicMeshMaterialData &
	{
		return m_materialDatas.at(p_index);
	}
	...
	DynamicMesh::DynamicMesh(RenderContext &p_render_ctx, const io::filesystem::Path &p_path) : m_renderCtx(&p_render_ctx), m_path(p_path), m_materials(p_render_ctx)
	{
		...
		if (scene->HasMaterials())
		{
			...
			m_materials.addMaterial(0, "Default");
	...
	auto DynamicMesh::_createMaterial(void *p_mat, uint32 p_mat_index, const io::filesystem::Path &p_parent_path) -> void
	{
		...
		auto &mat_data{m_materials.addMaterial(p_mat_index, material_name)};

		// Albedo/base colour
		{
			if (aiColor3D ai_colour; ai_mat->Get(AI_MATKEY_COLOR_DIFFUSE, ai_colour) == AI_SUCCESS)
				mat_data.albedoColour = {ai_colour.r, ai_colour.g, ai_colour.b};
			LOG_TRACE("\tAlbedo colour: {}", mat_data.albedoColour);
		}
		// Roughness
		{
			ai_mat->Get(AI_MATKEY_ROUGHNESS_FACTOR, mat_data.roughness);
			LOG_TRACE("\tRoughness: {}", mat_data.roughness);
		}
		// Metalness
		{
			ai_mat->Get(AI_MATKEY_REFLECTIVITY, mat_data.metalness);
			LOG_TRACE("\tMetalness: {}", mat_data.metalness);
		}
		...
		// Load albedo map
		{
			...
			if (has_albedo_map)
			{
				...
				if (albedo_map_path.has_value())
					...
					mat_data.albedoMap = m_renderCtx->getGlobals()->whiteImage();
			}
			else
			{
				LOG_WARN("\tMaterial '{}' does not have an albedo map", material_name);
		...
		// Load normal map
		{
			...
			if (ai_mat->GetTexture(aiTextureType_NORMALS, 0, &ai_normal_map_path) == AI_SUCCESS)
			{
				auto normal_map_path{get_path_and_create_texture_if_exists(ai_normal_map_path, "normal")};
				if (normal_map_path.has_value())
					mat_data.normalMap = m_renderCtx->createImageRef(*normal_map_path);
				mat_data.hasNormalMap = 1u;
			}
			else
			{
				mat_data.hasNormalMap = 0u;
				mat_data.normalMap    = m_renderCtx->getGlobals()->whiteImage();
				LOG_WARN("\tMaterial '{}' does not have a normal map", material_name);
			}
		}
	}
