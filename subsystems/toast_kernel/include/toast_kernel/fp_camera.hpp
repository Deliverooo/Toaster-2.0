...
namespace toaster
{
	...
	class TST_KERNEL_API FPCamera : public Camera
	{
	public:
		FPCamera() = default;
	...
	struct TST_KERNEL_API FirstPersonCameraEntityCreateParams
	{
		InputContext *p_ctx;
		float32       p_fov;
		float32       p_aspectRatio;
		float32       p_near;
		float32       p_far;
	};
	...
	public:
		...
		[[nodiscard]] auto XM_CALLCONV getViewMatrix() const -> Dx::XMMATRIX;
		[[nodiscard]] auto XM_CALLCONV getProjectionMatrix() const -> Dx::XMMATRIX;
		[[nodiscard]] auto XM_CALLCONV getRotationMatrix() const -> Dx::XMMATRIX; // Thank you very much -> https://vkguide.dev/docs/new_chapter_5/interactive_camera/
		[[nodiscard]] auto XM_CALLCONV getViewProjection() const -> Dx::XMMATRIX;
